...
int schedCount;
int rag[NLOCK + NPROC][NLOCK + NPROC]; // NEW
...
static void sysinit(void)
{
	...
	schedCount = 0; // init the resched count
	...
	for (int z = 0; z < NLOCK; z++)
	{
		locktab[z].state = LOCK_FREE;
		locktab[z].lock = FALSE;
		...
		locktab[z].wait_queue = lockQueue;
	}
